...
module.exports = app => {
  ...
  router.get('/news/list', async (req, res) => {
    ...
    // 聚合查询
    //这里file => settings => Languages&Frameworks => javascript =>Libraries =>download =>mongodb/mongoose=>download and install,
    //aggregate是mongodb的方法
    const cats = await Category.aggregate([
      { $match: { parent: parent._id } },
      {
        // 关联查询数据
        $lookup: {
           from: 'articles', //关联的表或者集合名字
          localField: '_id',//本地键
...
//导入英雄数据
router.get('/students/init',async (req,res)=>{
  ...
  for(let cat of rawData){
    ...
    cat.students = cat.students.map(student => {
      student.categories = [category]
      return student
    })
    //录入英雄
    await Student.insertMany(cat.students)
  }
  ...
  router.get('/students/list', async (req, res) => {
    ...
    const cats = await Category.aggregate([
      ...
      {
        $lookup: {
          from: 'students', //关联的表或者集合名字
          localField: '_id',//本地键
          foreignField: 'categories',
          as: 'studentList'//自己起的名
        }
      }
    ])
    const subCats = cats.map(v => v._id)
    cats.unshift({
      name: '热门',
      studentList: await Student.find().where({
        categories: { $in: subCats }
      }).limit(10).lean()
    })

    // cats.map(cat => {
    //   cat.studentList.map(news => {
    //     news.categoryName = (cat.name === '热门')
    //         ? news.categories[0].name : cat.name
    //     return news
    //   })
